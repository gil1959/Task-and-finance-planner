...
export function CategoryModal({ open, onOpenChange }: CategoryModalProps) {
  ...
  const handleAdd = async () => {
    try {
      ...
      alert("Gagal menambah kategori: " + (e.message || "Unknown error"))
    }
  }
  ...
  const handleUpdate = async (id: string) => {
    try {
      ...
      alert("Gagal memperbarui kategori: " + (e.message || "Unknown error"))
    }
  }
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="flex gap-2 items-center mb-6">
            ...
            <Button type="button" onClick={handleAdd} size="icon"><Plus className="h-4 w-4" /></Button>
          </div>
            ...
            {categories.map(cat => (
              <div key={cat.id} className="flex items-center justify-between p-2 border rounded-md">
                ...
                <div className="flex gap-1">
                  {editingId === cat.id ? (
                    <>
                      <Button type="button" variant="ghost" size="sm" onClick={() => handleUpdate(cat.id)}>Simpan</Button>
                      <Button type="button" variant="ghost" size="sm" onClick={() => setEditingId(null)}>Batal</Button>
                    </>
                  ) : (
                    <>
                      <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-blue-500" onClick={() => {
                        setEditingId(cat.id);
                      ...
                      <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-red-500" onClick={() => handleDelete(cat.id)}>
